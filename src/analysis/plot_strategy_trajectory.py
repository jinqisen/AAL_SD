...
def _extract_distribution_hints(
    ...
    return {
        "u_mean": _find(
            r"U\s*(?:分布)?\s*(?:均值|mean)\s*[^0-9\-]*\(?\s*([0-9]*\.?[0-9]+)\s*\)?"
        ),
        "u_p75": _find(
            r"U\s*(?:分布)?\s*75\s*(?:分位|percentile)\s*[^0-9\-]*\(?\s*([0-9]*\.?[0-9]+)\s*\)?"
        ),
        "k_mean": _find(
            r"K\s*(?:分布)?\s*(?:均值|mean)\s*[^0-9\-]*\(?\s*([0-9]*\.?[0-9]+)\s*\)?"
        ),
        "k_p75": _find(
            r"K\s*(?:分布)?\s*75\s*(?:分位|percentile)\s*[^0-9\-]*\(?\s*([0-9]*\.?[0-9]+)\s*\)?"
        ),
    }
...
def load_trace(trace_path: str) -> pd.DataFrame:
    ...
    def _extract_values(
        items: Any, key: str, limit: Optional[int] = None
    ) -> List[float]:
        ...
        for it in seq:
            if not isinstance(it, dict):
                raise TypeError(
                    f"Expected dict item for key={key}, got {type(it).__name__}"
                )
            v = it.get(key)
            if v is None:
                continue
    ...
    with open(trace_path, "r") as f:
        for line_no, line in enumerate(f, 1):
            ...
            if etype == "epoch_end":
                ...
                row.update(
                    {
                        ...
                        "sampler_type": entry.get("sampler_type")
                        or context.get("sampler")
                        or "unknown",
                ...
                state = (
                    entry.get("state") if isinstance(entry.get("state"), dict) else {}
                )
                row.update(
                    {
                ...
                selection = (
                    entry.get("selection")
                    if isinstance(entry.get("selection"), dict)
                    else {}
                )
                ranking = (
                    entry.get("ranking")
                    if isinstance(entry.get("ranking"), dict)
                    else {}
                )
                lambda_ctrl = (
                    entry.get("lambda_controller")
                    if isinstance(entry.get("lambda_controller"), dict)
                    else {}
                )
                training_state = (
                    entry.get("training_state")
                    if isinstance(entry.get("training_state"), dict)
                    else {}
                )
                row.update(
                    {
                        ...
                        "sampler_type": entry.get("sampler", {}).get("sampler_type")
                        if isinstance(entry.get("sampler"), dict)
                        else None,
                        "sampler_class": entry.get("sampler", {}).get("sampler_class")
                        if isinstance(entry.get("sampler"), dict)
                        else None,
                ...
                top_items = (
                    entry.get("top_items")
                    if isinstance(entry.get("top_items"), list)
                    else []
                )
                selected_items = (
                    entry.get("selected_items")
                    if isinstance(entry.get("selected_items"), list)
                    else []
                )
    ...
    if "selected_ids" in df.columns:
        df["selected_ids_count"] = df["selected_ids"].apply(
    ...
    if "l3_u_mean_topn" in df.columns:
        df["avg_uncertainty"] = df["avg_uncertainty"].fillna(
            pd.to_numeric(df["l3_u_mean_topn"], errors="coerce")
        )
    if "l3_k_mean_topn" in df.columns:
        df["avg_knowledge_gain"] = df["avg_knowledge_gain"].fillna(
            pd.to_numeric(df["l3_k_mean_topn"], errors="coerce")
        )
    if "l3_u_mean_selected" in df.columns:
        df["u_mean"] = df["u_mean"].fillna(
            pd.to_numeric(df["l3_u_mean_selected"], errors="coerce")
        )
    if "l3_k_mean_selected" in df.columns:
        df["k_mean"] = df["k_mean"].fillna(
            pd.to_numeric(df["l3_k_mean_selected"], errors="coerce")
        )
    if "l3_u_p75_selected" in df.columns:
        df["u_p75"] = df["u_p75"].fillna(
            pd.to_numeric(df["l3_u_p75_selected"], errors="coerce")
        )
    if "l3_k_p75_selected" in df.columns:
        df["k_p75"] = df["k_p75"].fillna(
            pd.to_numeric(df["l3_k_p75_selected"], errors="coerce")
        )
    ...
    with open(trace_path, "r") as f:
        for line in f:
            ...
            if entry.get("type") != "initialized":
                continue
            ab = (
                entry.get("ablation") if isinstance(entry.get("ablation"), dict) else {}
            )
            return {
                "lambda_policy": ab.get("lambda_policy")
                if isinstance(ab.get("lambda_policy"), dict)
                else None,
                "agent_threshold_overrides": ab.get("agent_threshold_overrides")
                if isinstance(ab.get("agent_threshold_overrides"), dict)
                else {},
            }
...
def add_lambda_policy_diagnostics(
    per_round_df: pd.DataFrame, trace_meta: Dict[str, Any]
) -> pd.DataFrame:
    ...
    overrides = (
        trace_meta.get("agent_threshold_overrides")
    ...
    risk_raw = pd.to_numeric(
        df.get("overfit_risk", pd.Series([float("nan")] * len(df))), errors="coerce"
    ).astype(float)
    ...
    for i in range(len(df)):
        ...
        rh = [
            x
            for x in risk_hist[max(0, len(risk_hist) - window) :]
            if math.isfinite(float(x))
        ]
        th = [
            x
            for x in tvc_hist[max(0, len(tvc_hist) - window) :]
            if math.isfinite(float(x))
        ]
    ...
    df["policy_overfit_risk_ema"] = pd.to_numeric(
        pd.Series(out_ema), errors="coerce"
    ).astype(float)
    df["policy_ci_risk_th_rawhist"] = pd.to_numeric(
        pd.Series(out_risk_th), errors="coerce"
    ).astype(float)
    df["policy_ci_risk_hit"] = pd.Series(out_risk_hit, dtype="boolean")
    df["policy_ci_tvc_th"] = pd.to_numeric(
        pd.Series(out_tvc_th), errors="coerce"
    ).astype(float)
...
def plot_run_lambda_compare(combined_round_df: pd.DataFrame, output_dir: str):
    ...
    pivot = df.pivot_table(
        index="round", columns="experiment_name", values="lambda_eff", aggfunc="mean"
    ).sort_index()
...
def plot_run_miou_compare(combined_round_df: pd.DataFrame, output_dir: str):
    ...
    if "round" not in combined_round_df.columns:
        return
    if (
        "val_miou" not in combined_round_df.columns
        and "final_miou" not in combined_round_df.columns
    ):
        return
    ...
    pivot = df.pivot_table(
        index="round", columns="experiment_name", values="val_miou", aggfunc="mean"
    ).sort_index()
    if pivot.empty or pivot.shape[1] == 0:
        return
...
def export_run_experiment_summary(combined_round_df: pd.DataFrame, output_dir: str):
    ...
    for name, g in df.groupby("experiment_name", dropna=False):
        ...
        best_miou = None
        if (
            "val_miou" in g.columns
            and pd.to_numeric(g["val_miou"], errors="coerce").notna().any()
        ):
        ...
        if not gl.empty:
            last_lambda = float(
                pd.to_numeric(gl["lambda_eff"], errors="coerce").iloc[-1]
            )
        rows.append(
            {
                ...
                "best_miou": best_miou,
                "lambda_mean": float(
                    pd.to_numeric(g["lambda_eff"], errors="coerce").mean()
                )
                if pd.to_numeric(g["lambda_eff"], errors="coerce").notna().any()
                else None,
                "lambda_last": last_lambda,
                "overfit_risk_mean": float(
                    pd.to_numeric(g.get("overfit_risk"), errors="coerce").mean()
                )
                if "overfit_risk" in g.columns
                else None,
                "tvc_neg_rate_mean": float(
                    pd.to_numeric(g.get("grad_tvc_neg_rate"), errors="coerce").mean()
                )
                if "grad_tvc_neg_rate" in g.columns
                else None,
            }
        )
    ...
    out = out.sort_values(
        ["last_miou", "best_miou"], ascending=False, na_position="last"
...
def plot_gradient_decomposition(df: pd.DataFrame, output_dir: str):
    ...
    plt.plot(
        df["round"], df[u_col], label=u_col, marker="s", linestyle="-", color="#1f77b4"
    )
    plt.plot(
        df["round"], df[k_col], label=k_col, marker="^", linestyle="-", color="#ff7f0e"
    )
...
def load_epoch_end(trace_path: str) -> pd.DataFrame:
    ...
    with open(trace_path, "r") as f:
        for line in f:
            ...
            grad = entry.get("grad") if isinstance(entry.get("grad"), dict) else {}
            cos_to_mean = (
                grad.get("cos_to_mean")
                if isinstance(grad.get("cos_to_mean"), dict)
                else {}
            )
            cos_consecutive = (
                grad.get("cos_consecutive")
                if isinstance(grad.get("cos_consecutive"), dict)
                else {}
            )
            total_norm = (
                grad.get("total_norm")
                if isinstance(grad.get("total_norm"), dict)
                else {}
            )
            rows.append(
                {
...
def aggregate_gradients_per_round(epoch_df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = out.merge(
        g["grad_train_val_cos"].mean().rename("grad_tvc_mean"),
        left_on="round",
        right_index=True,
        how="left",
    )
    out = out.merge(
        g["grad_train_val_cos"].median().rename("grad_tvc_median"),
        left_on="round",
        right_index=True,
        how="left",
    )
    out = out.merge(
        g["grad_train_val_cos"].min().rename("grad_tvc_min"),
        left_on="round",
        right_index=True,
        how="left",
    )
    out = out.merge(
        g["grad_train_val_cos"].max().rename("grad_tvc_max"),
        left_on="round",
        right_index=True,
        how="left",
    )
    out = out.merge(
        g["grad_train_val_cos"].apply(_last).rename("grad_tvc_last"),
        left_on="round",
        right_index=True,
        how="left",
    )
    ...
    out = out.merge(
        g.size().astype(int).rename("grad_epochs"),
        left_on="round",
        right_index=True,
        how="left",
    )
    ...
    out["grad_epochs"] = pd.to_numeric(out["grad_epochs"], errors="coerce").astype(
        "Int64"
    )
...
def _effective_lambda(per_round_df: pd.DataFrame) -> pd.Series:
    ...
    if "lambda_effective" in per_round_df.columns:
        base = base.combine_first(
            pd.to_numeric(per_round_df["lambda_effective"], errors="coerce")
        )
    if "lambda_policy_apply" in per_round_df.columns:
        base = base.combine_first(
            pd.to_numeric(per_round_df["lambda_policy_apply"], errors="coerce")
        )
    if "lambda_override" in per_round_df.columns:
        base = base.combine_first(
            pd.to_numeric(per_round_df["lambda_override"], errors="coerce")
        )
    if "lambda_t" in per_round_df.columns:
        base = base.combine_first(
            pd.to_numeric(per_round_df["lambda_t"], errors="coerce")
        )
...
def _rankdata(values: List[float]) -> List[float]:
    try:
        ...
        # Fallback to manual implementation if scipy not available
...
def _spearman(xs: List[float], ys: List[float]) -> Optional[float]:
    if len(xs) != len(ys) or len(xs) < 3:
        return None
    if any((not math.isfinite(x)) for x in xs) or any(
...
def _fixed_effect_residuals(
    values: List[float], effects: Dict[str, List[Any]]
) -> Optional[np.ndarray]:
    if not values:
        return None
...
def _partial_corr_fixed_effects(
    xs: List[float], ys: List[float], effects: Dict[str, List[Any]]
) -> Dict[str, Optional[float]]:
    ...
    yr_list = yr.tolist()
    return {
        "pearson": _pearson(xr_list, yr_list),
        "spearman": _spearman(xr_list, yr_list),
    }
    ...
    with open(trace_path, "r") as f:
        for line in f:
            ...
            if t == "round_summary":
                ...
                ts = (
                    ev.get("training_state")
                    if isinstance(ev.get("training_state"), dict)
                    else {}
                )
...
def deep_analyze_risk_miou(
    ...
    for run_id in run_ids:
        ...
        for exp in experiments:
            ...
            risks = [
                risk_by_round[r]
                for r in sorted(risk_by_round.keys())
                if r in risk_by_round
            ]
            risk_mean = float(np.mean(risks)) if risks else None
            risk_max = float(np.max(risks)) if risks else None
            risk_last = (
                float(risk_by_round.get(last_round))
                if last_round in risk_by_round
                else None
            )
            ...
            corr_risk_delta_next = (
                _pearson(risks_t, deltas_next) if len(risks_t) >= 3 else None
            )
            spr_risk_delta_next = (
                _spearman(risks_t, deltas_next) if len(risks_t) >= 3 else None
            )
...
def print_deep_risk_miou_report(df: pd.DataFrame) -> None:
    ...
    for xcol in ["overfit_mean", "overfit_max", "overfit_last"]:
        for ycol in ["last_miou", "best_miou"]:
            ...
            print(
                f"  {xcol:12s} vs {ycol:9s}  n={r['n']:3d}  pearson={r['pearson']}  spearman={r['spearman']}"
            )
    ...
    ys_best = (
        base["best_miou"].astype(float).tolist()
        if base["best_miou"].notna().all()
        else None
    )
    eff = {
        "seed": base["seed"].tolist(),
        "experiment_name": base["experiment_name"].tolist(),
    }
    r_last = _partial_corr_fixed_effects(xs, ys_last, eff)
    print(
        f"  overfit_mean vs last_miou | seed+experiment  pearson={r_last['pearson']}  spearman={r_last['spearman']}  n={len(xs)}"
    )
    if ys_best is not None:
        r_best = _partial_corr_fixed_effects(xs, ys_best, eff)
        print(
            f"  overfit_mean vs best_miou | seed+experiment  pearson={r_best['pearson']}  spearman={r_best['spearman']}  n={len(xs)}"
        )

    print("\nWithin-seed (across experiments) Spearman:")
    for seed, g in d.dropna(subset=["seed", "overfit_mean", "last_miou"]).groupby(
        "seed"
    ):
        xs = g["overfit_mean"].astype(float).tolist()
        ys = g["last_miou"].astype(float).tolist()
        print(
            f"  seed{int(seed):02d}  n={len(xs)}  spearman(overfit_mean,last_miou)={_spearman(xs, ys)}  pearson={_pearson(xs, ys)}"
        )

    print("\nPer-experiment (across seeds) Spearman:")
    for exp, g in d.dropna(
        subset=["experiment_name", "overfit_mean", "last_miou"]
    ...
    lag = d.dropna(
        subset=[
            "corr_overfit_to_next_delta_spearman",
            "corr_overfit_to_next_delta_pearson",
        ]
    )
    if lag.empty:
        ...
        by_exp = lag.groupby("experiment_name")[
            [
                "corr_overfit_to_next_delta_pearson",
                "corr_overfit_to_next_delta_spearman",
            ]
...
def add_overfit_signals(
    ...
    computed_risk = (
        (risk_w_neg_rate * neg_rate) + (risk_w_min * min_bad) + (risk_w_last * last_bad)
    )
    if risk_in_trace.notna().any():
        ...
        if bool(bad.any()):
            raise RuntimeError(
                "overfit_risk in trace is inconsistent with computed risk"
            )
    ...
    lam = _effective_lambda(df)
    df["lambda_eff"] = lam
    df["overfit_risk_next"] = pd.to_numeric(df["overfit_risk"], errors="coerce").shift(
        -1
    )
    return df
...
def export_lambda_diagnostics(
    per_round_df: pd.DataFrame, output_dir: str, name: str
) -> Optional[str]:
    if per_round_df.empty:
        return None
    cols = [
...
def plot_overfit_vs_gain(per_round_df: pd.DataFrame, output_dir: str, name: str):
    if per_round_df.empty:
        return
    if (
        "overfit_risk" not in per_round_df.columns
        or "miou_gain" not in per_round_df.columns
    ):
        return
...
def plot_lambda_vs_tvc(per_round_df: pd.DataFrame, output_dir: str, name: str):
    ...
    if lam.isna().all():
        return
    if (
        "grad_tvc_mean" not in per_round_df.columns
        and "grad_tvc_last" not in per_round_df.columns
    ):
        return
    tvc = (
        per_round_df["grad_tvc_mean"]
        if "grad_tvc_mean" in per_round_df.columns
        else per_round_df["grad_tvc_last"]
    )
...
def analyze_run_dir(
    run_dir: str, output_dir: str, experiment_name: Optional[str]
) -> None:
    if not os.path.isdir(run_dir):
    ...
    for fn in traces:
        ...
        corr_lam_tvc_mean = _corr(
            lam, per_round.get("grad_tvc_mean", pd.Series(dtype=float))
        )
        corr_lam_tvc_last = _corr(
            lam, per_round.get("grad_tvc_last", pd.Series(dtype=float))
        )
        corr_lam_neg_rate = _corr(
            lam, per_round.get("grad_tvc_neg_rate", pd.Series(dtype=float))
        )

        report_rows.append(
            {
                "experiment_name": name,
                "n_rounds": int(per_round["round"].notna().sum())
                if "round" in per_round.columns
                else int(len(per_round)),
                "lambda_corr_tvc_mean": corr_lam_tvc_mean,
                "lambda_corr_tvc_last": corr_lam_tvc_last,
                "lambda_corr_tvc_neg_rate": corr_lam_neg_rate,
                "tvc_mean_all_rounds": float(
                    pd.to_numeric(
                        per_round.get("grad_tvc_mean"), errors="coerce"
                    ).mean()
                )
                if "grad_tvc_mean" in per_round.columns
                else None,
                "tvc_neg_rate_all_rounds_mean": float(
                    pd.to_numeric(
                        per_round.get("grad_tvc_neg_rate"), errors="coerce"
                    ).mean()
                )
                if "grad_tvc_neg_rate" in per_round.columns
                else None,
                "tvc_min_any": float(
                    pd.to_numeric(per_round.get("grad_tvc_min"), errors="coerce").min()
                )
                if "grad_tvc_min" in per_round.columns
                else None,
                "tvc_max_any": float(
                    pd.to_numeric(per_round.get("grad_tvc_max"), errors="coerce").max()
                )
                if "grad_tvc_max" in per_round.columns
                else None,
            }
        )
        ...
        overfit_report_rows.append(
            {
                ...
                "overfit_risk_mean": float(
                    pd.to_numeric(per_round.get("overfit_risk"), errors="coerce").mean()
                )
                if "overfit_risk" in per_round.columns
                else None,
                "overfit_risk_max": float(
                    pd.to_numeric(per_round.get("overfit_risk"), errors="coerce").max()
                )
                if "overfit_risk" in per_round.columns
                else None,
                "corr_risk_miou_gain": _corr(
                    per_round.get("overfit_risk", pd.Series(dtype=float)),
                    per_round.get("miou_gain", pd.Series(dtype=float)),
                ),
                "corr_risk_miou_gain_next": _corr(
                    per_round.get("overfit_risk", pd.Series(dtype=float)),
                    per_round.get("miou_gain_next", pd.Series(dtype=float)),
                ),
                "corr_tvc_mean_miou_gain": _corr(
                    per_round.get("grad_tvc_mean", pd.Series(dtype=float)),
                    per_round.get("miou_gain", pd.Series(dtype=float)),
                ),
                "corr_tvc_mean_miou_gain_next": _corr(
                    per_round.get("grad_tvc_mean", pd.Series(dtype=float)),
                    per_round.get("miou_gain_next", pd.Series(dtype=float)),
                ),
            }
    ...
    if combined_round_rows:
        ...
        combined_round_path = os.path.join(
            output_dir, "run_all_experiments_per_round_with_grad.csv"
        )
    ...
    if combined_epoch_rows:
        ...
        combined_epoch_path = os.path.join(
            output_dir, "run_all_experiments_epoch_end_with_grad.csv"
        )
    ...
    if report_rows:
        ...
        cols = [
            ...
            for c in [
                "experiment_name",
                "n_rounds",
                "lambda_corr_tvc_mean",
                "lambda_corr_tvc_last",
                "tvc_mean_all_rounds",
                "tvc_neg_rate_all_rounds_mean",
                "tvc_min_any",
                "tvc_max_any",
            ]
    ...
    if overfit_report_rows:
        overfit = (
            pd.DataFrame(overfit_report_rows)
            .sort_values("experiment_name")
            .reset_index(drop=True)
        )
...
def _resolve_trace_path(
    experiment_dir_or_trace: str, experiment_name: Optional[str]
) -> Optional[str]:
    if os.path.isfile(experiment_dir_or_trace) and experiment_dir_or_trace.endswith(
        ".jsonl"
    ):
    ...
    candidates = [
        ...
        if fn.endswith("_trace.jsonl")
        and os.path.isfile(os.path.join(experiment_dir_or_trace, fn))
    ]
    ...
    print(
        "Error: multiple *_trace.jsonl found; please pass --experiment_name. Candidates:"
    )
    for c in candidates:
        print(f"- {c}")
    return None
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Experiment name (used to pick <exp>_trace.jsonl)",
    )
    parser.add_argument(
        "--output", type=str, default=None, help="Output directory for plots/exports"
    )
    parser.add_argument(
        "--export_csv", action="store_true", help="Export per-round summary CSV"
    )
    parser.add_argument(
        "--grad_report",
        action="store_true",
        help="Analyze grad alignment across all *_trace.jsonl in a run dir",
    )
    parser.add_argument(
        "--risk_miou_deep",
        action="store_true",
        help="Deep analyze risk↔mIoU across multiple runs/seeds",
    )
    parser.add_argument(
        "--runs_root",
        type=str,
        default="results/runs",
        help="Root directory containing run_id folders",
    )
    parser.add_argument(
        "--seeds",
        type=int,
        nargs="*",
        default=None,
        help="Seed list for building run_ids",
    )
    parser.add_argument(
        "--run_id_template",
        type=str,
        default="run_src_full_model_with_baselines_seed{seed}",
        help="Template for run_id",
    )
    parser.add_argument(
        "--run_ids",
        type=str,
        nargs="*",
        default=None,
        help="Explicit run_ids (overrides --seeds)",
    )
    parser.add_argument(
        "--experiments",
        type=str,
    ...
    if args.export_csv:
        name = args.experiment_name or os.path.basename(trace_path).replace(
            "_trace.jsonl", ""
        )
